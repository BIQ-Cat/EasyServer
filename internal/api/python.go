...
func CreateModule(pModule *C.PyObject, pLib *C.PyObject) (module easyserver.Module, ok bool) {
	defer C.PyErr_Print()
	...
	for C.PyDict_Next(pRoute, &pos, &pKey, &pController) != 0 {
		...
		controller.Handler = handlePython(pHandler, pLib)
...
func handlePython(pHandler *C.PyObject, pLib *C.PyObject) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if C.PyTuple_SetItem(pRequestArgs, 4, pContext) == -1 {
			return
		}
		...
		pRecieve := C.PyObject_GetAttrString(pResponse, C.CString("recieve"))
		if pRecieve == nil {
			return
		}
		defer C.Py_DECREF(pRecieve)

		pRecieveArgs := C.PyTuple_New(0)
		if pRecieveArgs == nil {
			return
		}
		defer C.Py_DECREF(pRecieveArgs)

		pData := C.PyObject_Call(pRecieve, pRecieveArgs, nil)
		if pData == nil {
			return
		}
		defer C.Py_DECREF(pData)

		data := C.PyBytes_AsString(pData)
		if data == nil {
			return
		}

		io.WriteString(w, C.GoString(data))
	})
}
